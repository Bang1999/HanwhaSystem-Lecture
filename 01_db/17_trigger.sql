/* trigger(트리거) */
-- 테이블이 2개 있는데 insert, update, delete, 이벤트가 일어나면 다른테이블에서 어떠한 일이 일어나는지
-- (하나의 트랜잭션이 2개의 테이블에 일어날때 트리거를 걸어준다.)
-- ex) 물건 구매시 재고 상태

-- // : 프로시저가 끝나는거다. 문자의 끝, (구분선)
-- 프로시저 : 절차 지향 언어
...
CREATE OR REPLACE TRIGGER after_order_menu_insert
	...
	FOR EACH ROW
BEGIN
	UPDATE tbl_order					-- total_order_price : 유도속성
...
SELECT CAST(YEAR(NOW()) AS VARCHAR(4));
SELECT CAST(LPAD(MONTH(NOW()), 2, 0) AS VARCHAR(2));
SELECT CAST(MONTH(NOW()) AS VARCHAR(2));
...
CREATE TABLE pro_detail(
  ...
  FOREIGN KEY(pcode) REFERENCES product(pcode)				-- (pcode) 생략가능(어차피 pk가 넘어올거라서)
);																			-- 부모 테이블의 pk가 넘어오는게 	
																			-- 당연하므로 생략 가능
...
-- 존재하는 상품들에 대한 입출고 진행(insert 할꺼다)
INSERT
  INTO pro_detail
(
  dcode, pcode, pdate
